...
% Define your tests using plunit
:- begin_tests(powerI).

test(emptySet, [nondet]) :-
    powerI([], Result),
    Result = [[]].
...
test(multiple_elements, [nondet]) :-
    powerI([1, 2, 3], Result),
    Result = [[], [1], [2], [1, 2], [3], [1, 2, 3]].
...
/*
1. Tests for union and power.
2. Tests for uniion doesnt have dups.
3. my implementation are correct for the codes i made.
4. How to check that the powersets from two different valid operations are same. Sort it simply.
*/